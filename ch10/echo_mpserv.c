...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        if (pid == -1)
        {
            close(clnt_sock); // 父进程关闭客户端套接字，因为 fork 失败，没有子进程创建
            continue;
        }
        if (pid == 0) // 子进程运行区域,此部分向客户端提供回声服务
        {
            close(serv_sock); // 子进程关闭服务器套接字，因为子进程不负责监听
            while ((str_len = read(clnt_sock, buf, BUFSIZ)) != 0)
                write(clnt_sock, buf, str_len);

            close(clnt_sock); // 子进程关闭客户端套接字，表示回声服务完成
            puts("client disconnected...");
            return 0;
        }
        else
            close(clnt_sock); // 父进程关闭客户端套接字，因为父进程不直接处理客户端数据
    }
    close(serv_sock); // 父进程关闭服务器套接字，在程序结束时
